...
kops create cluster \
--name=k8s.westys.net --state=s3://k8s.westys.net --dns-zone=westys.net \
--zones="eu-west-1a,eu-west-1b,eu-west-1c" --network-cidr=172.10.0.0/16 \
--node-count=1 --master-count=1 --node-size=t2.small --master-size=t2.micro \
--node-volume-size=16 --master-volume-size=8 \
